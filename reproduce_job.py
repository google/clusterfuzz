...
"""Reproduces testcases for an OSS-Fuzz job locally."""
...
@click.option('--job', required=True, help='Specific job type to filter by (e.g., libfuzzer_asan_google).')
@click.option(
    '--config-dir',
    '-c',
    required=False,
...
def cli(project_name, job, config_dir, parallelism, os_version, environment,
        local_build_path, engine, sanitizer, use_batch, gcs_bucket, limit, log_dir, testcase_id):
  """
  Reproduces testcases for an OSS-Fuzz job, saving logs to files.
  """
  ...
  click.echo(f"Fetching testcases for {project_name} and job {job}...")
  try:
    with ndb_init.context():
      query = data_types.Testcase.query(
          data_types.Testcase.project_name == project_name,
          data_types.Testcase.job_type == job,
  ...
  if not testcases:
    click.secho(f'No open testcases found for {project_name} and job {job}.', fg='yellow')
    return
