...
class LibFuzzerMinimizeTaskTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    helpers.patch(self, [
        '_internal.bot.tasks.minimize_task._run_libfuzzer_testcase',
        '_internal.bot.tasks.minimize_task._run_libfuzzer_tool',
    ])
...
class MinimizeTaskTestUntrusted(
  ...
  def setUp(self):
    ...
    patcher = mock.patch(
        '_internal.bot.fuzzers.libFuzzer.fuzzer.LibFuzzer.fuzzer_directory',
    ...
    mock_fuzzer_directory.return_value = os.path.join(
        environment.get_value('ROOT_DIR'), 'src', '_internal', 'bot', 'fuzzers',
        'libFuzzer')
  ...
  def test_minimize(self):
    """Test minimize."""
    helpers.patch(self, ['_internal.base.utils.is_oss_fuzz'])
