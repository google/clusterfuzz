...
from _internal.bot.fuzzers.libFuzzer import constants as libfuzzer_constants
from _internal.bot.fuzzers.libFuzzer import engine as libfuzzer_engine
...
class ConvertDependencyUrlToLocalPathTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    test_helpers.patch(self, [
        '_internal.bot.webserver.http_server.get_absolute_testcase_file',
        '_internal.system.environment.platform',
        '_internal.base.utils.normalize_path',
    ])
...
class GetResourcePathsTest(unittest.TestCase):
  ...
  def setUp(self):
    test_helpers.patch(self, [
        '_internal.bot.testcase_manager.convert_dependency_url_to_local_path',
    ])
