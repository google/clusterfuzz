    ...
    if task_name != 'fuzz':
      ...
      if testcase:
        ...
        if (task_name != 'variant' and testcase_platform_id and
          ...
          logs.log(
              'Testcase %d platform (%s) does not match with ours (%s), checking ...'
          ...
          if not (environment.is_testcase_deprecated(testcase_platform_id)
            and environment.can_testcase_run_on_platform(testcase_platform_id, current_platform_id)):
            logs.log(
          ...
          logs.log(
              'Testcase %d platform (%s) can run on current platform (%s)'
              % (testcase.key.id(), testcase_platform_id, current_platform_id))
