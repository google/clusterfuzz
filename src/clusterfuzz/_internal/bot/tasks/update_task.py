...
def get_source_url():
  ...
  deployment_filename = utils.get_platform_deployment_filename(
      platform_name, release)
  return _deployment_file_url(deployment_filename)
...
def get_newer_source_revision():
  ...
  logs.info(f'Local source version: {local_source_version} - {release}.')
  logs.info(f'Remote source version: {source_version} - {release}.')
  ...
  logs.info(
      f'New source: {source_version} (from {local_source_version} - {release})')
  return source_version
...
def update_source_code():
  ...
  release = utils.get_clusterfuzz_release()
  logs.info(f'Source updated to {source_version} (release: {release}).')
