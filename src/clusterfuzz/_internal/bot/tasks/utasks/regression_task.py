...
def check_latest_revisions(
    ...
    fuzz_target: Optional[data_types.FuzzTarget],
    output: uworker_msg_pb2.RegressionTaskOutput,
  ...
  Args:
    ...
    output: Output argument. Any bad builds encountered while searching for the
      earliest good build are appended to `build_data_list`.
  ...
  for revision in reversed(revision_range[-EXTREME_REVISIONS_TO_TEST - 1:-1]):
    ...
    if not is_crash:
      ...
      return uworker_msg_pb2.Output(regression_task_output=output)
...
def find_earliest_good_revision(
  ...
  return uworker_msg_pb2.Output(regression_task_output=regression_task_output)
