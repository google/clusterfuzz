...
def _evict_build(current_build_dir):
  ...
  logs.log(
      f'Deleting build {least_recently_used.base_build_dir} to save space.')
...
class Build(BaseBuild):
  ...
  def _unpack_build(self,
    ...
    log_func(f'Build took {elapsed_time / 60.0:0.02f} minutes to unpack.')
...
def setup_build(revision=0, target_weights=None):
  ...
  for env_var in DEFAULT_BUILD_BUCKET_PATH_ENV_VARS:
    ...
    if bucket_path:
      ...
      logs.log(f'Bucket path not found for {env_var}')
