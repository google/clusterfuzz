...
FORWARDED_ATTRIBUTES: tuple[str, ...] = ('crash_state', 'crash_type',
                                         'group_id', 'one_time_crasher_flag',
                                         'project_name', 'security_flag',
                                         'timestamp', 'job_type', 'fuzzer_name')

VARIANT_CRASHES_IGNORE: re.Pattern[str] = re.compile(
    r'^(Out-of-memory|Timeout|Missing-library|Data race|GPU failure)')

VARIANT_STATES_IGNORE: re.Pattern[str] = re.compile(r'^NULL$')

VARIANT_THRESHOLD_PERCENTAGE: float = 0.2
VARIANT_MIN_THRESHOLD: int = 5
VARIANT_MAX_THRESHOLD: int = 10

TOP_CRASHES_LIMIT: int = 10

DELETE_TESTCASES_FROM_GROUPING: bool = local_config.ProjectConfig().get(
    'deduplication.delete_testcases_from_grouping', True)
GROUP_MAX_TESTCASE_LIMIT: Any = local_config.ProjectConfig().get(
    'deduplication.group_max_limit', 25)
CRASH_COMPARER_THRESHOLD: Any = local_config.ProjectConfig().get(
    'deduplication.crash_comparer_threshold')
CRASH_COMPARER_SAME_FRAMES: Any = local_config.ProjectConfig().get(
...
def _get_new_group_id() -> int:
  """Get a new group id for testcase grouping."""
  new_group = data_types.TestcaseGroup()
  new_group.put()
  return cast(int, new_group.key.id())


def is_same_variant(variant1: Any, variant2: Any) -> bool:
...
def matches_top_crash(testcase: TestcaseAttributes,
                      top_crashes_by_project_and_platform: dict[str, dict[
...
def _check_variant_grouping_candidate(
    testcase_1: TestcaseAttributes, testcase_2: TestcaseAttributes,
    grouping_candidates: dict[str, list[tuple[int, int]]]) -> None:
...
def _group_testcases_based_on_variants(
  ...
  grouping_candidates: dict[str, list[tuple[int, int]]] = (
      collections.defaultdict(list))
  project_num_testcases: dict[str, int] = collections.defaultdict(int)
  ...
  for project, candidate_list in grouping_candidates.items():
    project_ignore_testcases: set[int] = set()
    # Count the number of times a testcase is matched for grouping.
    project_counter: dict[int, int] = collections.defaultdict(int)
...
def _group_testcases_with_same_issues(
    testcase_map: dict[int, TestcaseAttributes]) -> None:
...
def _compare_testcases_crash_states(testcase_1: TestcaseAttributes,
                                    testcase_2: TestcaseAttributes) -> bool:
...
def _group_testcases_with_similar_states(
    testcase_map: dict[int, TestcaseAttributes]) -> None:
...
def _has_testcase_with_same_params(
    testcase: data_types.Testcase,
    testcase_map: dict[int, TestcaseAttributes]) -> bool:
...
def _shrink_large_groups_if_needed(
    testcase_map: dict[int, TestcaseAttributes]) -> None:
  """Shrinks groups that exceed a particular limit."""
  group_overflow: dict[tuple[str, str], int] = {}
  ...
  def _key_func(testcase: TestcaseAttributes) -> int:
    weight = 0
  ...
  group_id_with_testcases_map: dict[int, list[TestcaseAttributes]] = {}
  for testcase in testcase_map.values():
...
def _get_testcase_attributes(
    testcase: data_types.Testcase, testcase_map: dict[int, TestcaseAttributes],
    cached_issue_map: dict[str, dict[int, int]]) -> None:
  """Retrieve testcase attributes for grouping and add it to the map."""
  testcase_id = cast(int, testcase.key.id())
  ...
  if testcase.bug_information:
    ...
    if (project_name in cached_issue_map and
        issue_id in cached_issue_map[cast(str, project_name)]):
      testcase_attributes.issue_id = cached_issue_map[cast(
          str, project_name)][issue_id]
    else:
      try:
      ...
      try:
        ...
        original_issue_id = int(cast(Any, issue).id)
      ...
      if project_name not in cached_issue_map:
        cached_issue_map[cast(str, project_name)] = {}
      cached_issue_map[cast(str, project_name)][issue_id] = original_issue_id
      cached_issue_map[cast(
          str, project_name)][original_issue_id] = original_issue_id
      testcase_attributes.issue_id = original_issue_id


def group_testcases() -> None:
  """Group testcases based on rules like same bug numbers, similar crash
  states, etc."""
  testcase_map: dict[int, TestcaseAttributes] = {}
  cached_issue_map: dict[str, dict[int, int]] = {}
