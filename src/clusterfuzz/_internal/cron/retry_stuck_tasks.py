...
This script identifies testcases that have entered the analysis pipeline but have
stalled due to transient errors or other issues. It makes them idempotent by
marking restart attempts and validates data integrity by ensuring associated
jobs still exist before attempting to create new analysis tasks.
This version performs all filtering and categorization upfront before taking
any remediation actions, providing a clear and verbose summary of its findings.
"""
...
def _get_script_config() -> ScriptConfig:
    ...
    return ScriptConfig(
        stuck_deadline=stuck_deadline, cooldown_deadline=cooldown_deadline
    )
...
def _get_stuck_testcase_candidates_query(
    ...
    return data_types.Testcase.query(
        ...
        data_types.Testcase.one_time_crasher_flag == False,
...
def _is_job_valid(testcase: data_types.Testcase) -> bool:
    ...
    """
    job_exists_query = data_types.Job.query(data_types.Job.name == testcase.job_type)
...
def _is_in_cooldown(
    testcase: data_types.Testcase, cooldown_deadline: datetime.datetime
) -> bool:
    """
    Checks if a restart was recently attempted for this testcase by this cron.
    """
    last_attempt_time = testcase.get_metadata("retry_stuck_task_last_attempt_time")
    if last_attempt_time and last_attempt_time > cooldown_deadline:
        logs.info(
            f"Skipping testcase {_get_testcase_id(testcase)} because a restart was already "
            f"attempted at {last_attempt_time}."
        )
        return True
    return False
...
def _restart_analysis_for_testcases(
    ...
    for testcase in testcases_to_restart:
        ...
        logs.warning(
            f'Retriggering "minimize" for stuck testcase {testcase_id}. '
...
def _filter_and_categorize_candidates(
    candidates: List[data_types.Testcase], cooldown_deadline: datetime.datetime
...
def _log_verbose_summary(total_from_query: int, results: CategorizedTestcases) -> None:
    """
...
def main():
    ...
    restarted_count = _restart_analysis_for_testcases(categorized_results.to_restart)
