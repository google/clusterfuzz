...
def get_issue_description(testcase: data_types.Testcase,
  ...
  content_string += 'Crash State:\n%s\n' % (
      utils.indent_string(cast(str, crash_state) + '\n', 2))
...
def get_stacktrace(testcase: data_types.Testcase,
  ...
  if environment.is_running_on_app_engine():
    ...
    return str(cast(bytes, blobs.read_key(key)), 'utf-8', errors='replace')

  key = result[len(data_types.BLOBSTORE_STACK_PREFIX):]
  tmpdir = cast(str, environment.get_value('BOT_TMPDIR'))
  tmp_stacktrace_file = os.path.join(tmpdir, 'stacktrace.tmp')
...
def update_task_status(task_name: str,
  ...
  if expiry_interval is None:
    expiry_interval = cast(float | int | None,
                           environment.get_value('TASK_LEASE_SECONDS'))
    if expiry_interval is None:
      logs.error('expiry_interval is None and TASK_LEASE_SECONDS not set.')

  def _try_update_status() -> bool:
    """Try update metadata."""
    task_status = cast(data_types.TaskStatus,
                       get_task_status(task_name, create_if_needed=True))
    ...
    if (status == data_types.TaskState.STARTED and
        ...
        not dates.time_has_expired(
            cast(datetime.datetime, task_status.time),
            seconds=cast(int, expiry_interval) - 1)):
      return False
  ...
  while True:
    try:
      return cast(bool,
                  cast(Any, ndb).transaction(_try_update_status, retries=0))
    except Exception:
      ...
      time.sleep(utils.random_number(1, cast(int, failure_wait_interval)))
...
def update_heartbeat(force_update: bool = False) -> int:
  ...
  try:
    ...
    cast(Any, heartbeat).key = ndb.Key(data_types.Heartbeat, bot_name)
...
def bot_run_timed_out() -> bool:
  ...
  start_time = datetime.datetime.utcfromtimestamp(cast(float, start_time))
  ...
  actual_run_timeout = (
      cast(float, run_timeout) - cast(float, average_task_duration))

  return dates.time_has_expired(
      start_time, seconds=cast(int, actual_run_timeout))
...
def create_notification_entry(testcase_id: int | str, user_email: str) -> None:
  ...
  notification.testcase_id = cast(int | None, testcase_id)
...
def get_or_create_multi_entities_from_keys(
  ...
  return cast(list[_ModelT], all_entities)
...
def get_fuzzing_engines() -> list[str]:
  ...
  return [cast(str, f.engine) for f in ndb_utils.get_all_from_query(query)]
...
def get_all_job_type_names(project: str | None = None) -> list[str]:
  ...
  return sorted([cast(str, job.name) for job in query])
