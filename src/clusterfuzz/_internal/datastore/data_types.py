...
def clone_entity(e: _ModelT, **extra_args: Any) -> _ModelT:
  ...
  properties = cast(dict[str, Any], ent_class._properties)
  props = {
...
class Testcase(Model):
  ...
  def has_impacts(self) -> bool:
    return self.is_chromium() and not bool(self.one_time_crasher_flag)
  ...
  def is_status_unreproducible(self) -> bool:
    return bool(self.status and self.status.startswith('Unreproducible'))
  ...
  def _ensure_metadata_is_cached(self) -> None:
    ...
    try:
      cache = json_utils.loads(self.additional_metadata or '')
  ...
  def actual_fuzzer_name(self) -> str:
    ...
    return self.overridden_fuzzer_name or self.fuzzer_name or ''
...
class ReportMetadata(Model):
  ...
  serialized_crash_stack_frames = ndb.BlobProperty(default=b'', indexed=False)
...
class FuzzTarget(Model):
  ...
  def fully_qualified_name(self) -> str:
    ...
    return fuzz_target_fully_qualified_name(
        cast(str, self.engine), self.project, cast(str, self.binary))

  def project_qualified_name(self) -> str:
    """Get the name qualified by project."""
    return fuzz_target_project_qualified_name(self.project,
                                              cast(str, self.binary))
  ...
  def _pre_put_hook(self) -> None:
    ...
    self.key = ndb.Key(
        ...
        coverage_information_key(
            cast(str, self.fuzzer),
            cast(datetime.date | datetime.datetime, self.date)))
