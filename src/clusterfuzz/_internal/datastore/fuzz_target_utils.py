...
def get_fuzz_targets_for_target_jobs(
  ...
  return cast(list[data_types.FuzzTarget | None],
              ndb_utils.get_multi(target_keys))
...
def get_fuzz_target_jobs(
  ...
  if limit is not None:
    return cast(Iterator[data_types.FuzzTargetJob], query.iter(limit=limit))

  return cast(Iterator[data_types.FuzzTargetJob],
              ndb_utils.get_all_from_query(query))
