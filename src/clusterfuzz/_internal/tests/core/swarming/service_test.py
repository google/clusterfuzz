...
class SwarmingTest(unittest.TestCase):
  """Tests for swarming utility functions."""

  def setUp(self):
    helpers.patch(self, [
        'clusterfuzz._internal.base.utils.post_url',
        'clusterfuzz._internal.swarming.service._get_task_name',
        'clusterfuzz._internal.google_cloud_utils.credentials.get_default',
        'google.auth.transport.requests.Request',
        'clusterfuzz._internal.swarming.service.FeatureFlags',
    ])
  ...
  def test_get_spec_from_config_with_docker_image(self):
    ...
    job.put()
    spec = service._get_new_task_spec(  # pylint: disable=protected-access
  ...
  def test_get_spec_from_config_raises_error_on_unknown_config(self):
    ...
    with self.assertRaises(ValueError):
      service._get_new_task_spec(  # pylint: disable=protected-access
          'corpus_pruning', job.name, 'https://download_url')
  ...
  def test_push_swarming_task(self):
    ...
    job = data_types.Job(name='libfuzzer_chrome_asan', platform='LINUX')
    job.put()
    service.push_swarming_task('fuzz', 'https://download_url', job.name)
    ...
    self.mock.get_default.assert_called_with(service._SWARMING_SCOPES)  # pylint: disable=protected-access
    expected_headers = {
  ...
  def test_is_swarming_task(self):
    ...
    job.put()
    self.assertTrue(service.is_swarming_task('fuzz', job.name))

    job.environment_string = 'IS_SWARMING_JOB = False'
    job.put()
    self.assertFalse(service.is_swarming_task('fuzz', job.name))
  ...
  def test_get_task_dimensions_with_env_var(self):
    ...
    job = data_types.Job(name='libfuzzer_chrome_asan', platform='LINUX')
    dimensions = service._get_task_dimensions(job, [])  # pylint: disable=protected-access
...
class SwarmingServiceTest(unittest.TestCase):
  ...
  def setUp(self):
    helpers.patch(self, [
        'clusterfuzz._internal.swarming.service.is_swarming_task',
        'clusterfuzz._internal.swarming.service.push_swarming_task',
