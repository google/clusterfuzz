...
class UtaskIntegrationTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.testcase.fuzzed_keys = blobs.write_blob(os.path.join(
        TEST_LIBS_DATA_DIR, 'crash-adc83b19e793491b1c6ea0fd8b46cd9f32e592fc'))
    self.testcase.absolute_path = '/mnt/scratch0/clusterfuzz/bot/inputs/fuzzer-testcases/input.test'
    self.testcase.put()
    metadata = data_types.TestcaseUploadMetadata(testcase_id=self.testcase.key.id())
    metadata.put()
