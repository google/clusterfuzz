...
from internal.bot.fuzzers import options
from internal.bot.fuzzers.libFuzzer import engine as libFuzzer_engine
from internal.bot.tasks import commands
from internal.bot.tasks import corpus_pruning_task
...
from internal.google_cloud_utils import gsutil
...
class BaseTest(object):
  ...
  def setUp(self):
    ...
    helpers.patch(self, [
        ...
        'internal.bot.tasks.corpus_pruning_task.choose_cross_pollination_strategy',
        'internal.bot.tasks.task_creation.create_tasks',
        'internal.bot.tasks.setup.update_fuzzer_and_data_bundles',
        ...
        'internal.google_cloud_utils.blobs.write_blob',
...
class CorpusPruningTest(unittest.TestCase, BaseTest):
  ...
  def setUp(self):
    BaseTest.setUp(self)
    helpers.patch(self, [
        'internal.build_management.build_manager.setup_build',
        'internal.base.utils.get_application_id',
    ])
...
class CorpusPruningTestUntrusted(
  ...
  def setUp(self):
    ...
    helpers.patch(self, [
        'internal.bot.tasks.setup.get_fuzzer_directory',
        'internal.base.tasks.add_task',
        'internal.bot.tasks.corpus_pruning_task._record_cross_pollination_stats',
        'lib.clusterfuzz.fuzz.engine.get',
    ])

    self.mock.get.return_value = libFuzzer_engine.LibFuzzerEngine()
    self.mock.get_fuzzer_directory.return_value = os.path.join(
        environment.get_value('ROOT_DIR'), 'src', 'clusterfuzz', 'bot', 'fuzzers',
        'libFuzzer')
