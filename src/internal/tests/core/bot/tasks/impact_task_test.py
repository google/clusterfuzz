...
from internal.bot.tasks import impact_task
...
class ExecuteTaskTest(unittest.TestCase):
  ...
  def setUp(self):
    helpers.patch(self, [
        ...
        'internal.bot.tasks.impact_task.get_impacts_from_url',
        'internal.bot.tasks.impact_task.get_impacts_on_prod_builds',
        'internal.bot.tasks.setup.setup_testcase',
        'internal.build_management.build_manager.is_custom_binary',
        'internal.build_management.build_manager.has_production_builds',
        'internal.bot.testcase_manager.get_command_line_for_application',
        'internal.base.tasks.add_task',
    ])
...
class GetImpactsFromUrlTest(ComponentRevisionPatchingTest):
  ...
  def setUp(self):
    ...
    helpers.patch(self, [
        'internal.bot.tasks.impact_task.get_start_and_end_revision',
        'internal.bot.tasks.impact_task.get_impact',
        'internal.build_management.revisions.get_build_to_revision_mappings',
        'internal.build_management.revisions.revision_to_branched_from',
        'internal.datastore.data_handler.get_component_name',
    ])
...
class GetImpactsOnProdBuilds(unittest.TestCase):
  ...
  def setUp(self):
    helpers.patch(self, [
        'internal.bot.tasks.impact_task.get_impact_on_build',
        'internal.bot.tasks.impact_task.get_impacts_from_url',
        'internal.bot.testcase_manager.get_command_line_for_application',
    ])
...
class GetImpactOnBuild(unittest.TestCase):
  ...
  def setUp(self):
    helpers.patch(self, [
        'internal.build_management.build_manager.setup_production_build',
        'internal.system.environment.get_value',
        'internal.bot.testcase_manager.get_command_line_for_application',
        'internal.bot.testcase_manager.test_for_crash_with_retries',
    ])
    self.env = {
        'APP_PATH': 'app',
        'TEST_TIMEOUT': '9',
...
class GetStartAndEndRevisionTest(unittest.TestCase):
  ...
  def setUp(self):
    helpers.patch(self, [
        'internal.build_management.revisions.get_start_and_end_revision',
        'internal.build_management.revisions.get_component_range_list',
        'internal.bot.testcase_manager.get_command_line_for_application',
        'internal.system.environment.is_android',
    ])
