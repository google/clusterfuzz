...
class LibFuzzerMinimizeTaskTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    helpers.patch(self, [
        'internal.bot.tasks.minimize_task._run_libfuzzer_testcase',
        'internal.bot.tasks.minimize_task._run_libfuzzer_tool',
    ])
...
class MinimizeTaskTestUntrusted(
  ...
  def setUp(self):
    ...
    patcher = mock.patch(
        'internal.bot.fuzzers.libFuzzer.fuzzer.LibFuzzer.fuzzer_directory',
  ...
  def test_minimize(self):
    """Test minimize."""
    helpers.patch(self, ['internal.base.utils.is_oss_fuzz'])
