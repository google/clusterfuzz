...
# testcases = {1: {2: r2, 3: r3}, 2:{1:r2, 5:t5}}
...
def combine_testcases_into_group(
  ...
  add_testcases_to_group_map(group_map, group_id_to_reuse, testcase_1.id,
                             testcase_2.id, reason)
  for tc, sim_tcs in group_map[group_id_to_move].testcases.items():
    testcase_map[tc].group_id = group_id_to_reuse
    for sim_tc, r in sim_tcs.items():
      add_testcases_to_group_map(group_map, group_id_to_reuse, tc, sim_tc, r)
    ...
    if testcase.group_id == group_id_to_move:
      ...
      for tc_sim, r in group_map[group_id_to_move].testcases[
        ...
        add_testcases_to_group_map(group_map, group_id_to_reuse, testcase.id,
                                  tc_sim, r)
        testcase_map[tc_sim].group_id = group_id_to_reuse
...
def _get_group_testcases(group_id):
  keys = ndb_utils.get_all_from_query(
      data_types.Testcase.query(
          data_types.Testcase.group_id == group_id),
      keys_only=True)
  for key in keys:
    yield key.id()


def load_testcases(local_dir: str, group_id: int | None = None, testcase_ids: list[int] | None = None):
...
def _parse_grouper_args(script_args):
  steps = []
  group = 0
  for arg in script_args:
    if arg.startswith('steps='):
      steps = arg.removeprefix('steps=').split(',')
    elif arg.startswith('group='):
      group = int(arg.removeprefix('group='))
  return steps, group

def execute(args):
  """Load testcases and/or run grouper locally."""
  steps, group = _parse_grouper_args(args.script_args)
  ...
  if 'load' in steps:
    ...
    try:
      ...
      load_testcases(local_dir, group_id=group_id)
