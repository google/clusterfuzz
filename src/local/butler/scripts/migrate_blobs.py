...
def _migrate_gcs_blob(source_blob_key):
  '''Migrates blobs in the new GCS scheme.'''
...
def _migrate_legacy_blob(source_blob_key):
  ...
  _copy_blob(f'gs:/{source_blob_path}',
             f'gs://{staging_blob_bucket}/{new_blob_key}')
  return new_blob_key
...
def migrate_fuzzer(fuzzer):
  ...
  fuzzer.put()
  print(f'Migrated {fuzzer.name} to gcs://{staging_blob_bucket}/{new_blob_key}')
...
def migrate_job(job):
  ...
  job.put()
  print(
      f'Migrated job {job.name} to gcs://{staging_blob_bucket}/{new_blob_key}')
