...
class AFLEngine(engine.Engine):
  ...
  def fuzz(self, target_path, options, reproducers_dir, max_time):
   ...
   Returns:
    ...
    config = AflConfig.from_target_path(target_path)
  ...
  def reproduce(self, target_path, input_path, arguments, max_time):
    ...
    runner = prepare_runner(target_path, config, input_path, input_directory)

    reproduce_result = run_single_testcase(runner, input_path)
...
class AflConfig(object):
  ...
  def __init__(self):
    """Sets the configs to sane defaults.

    Use from_target_path if you want to
  ...
  def from_target_path(cls, target_path):
    """Instantiates and returns an AFLConfig object.
...
class AflFuzzInputDirectory(object):
  ...
  def create_new_if_needed(self):
    """Checks if any inputs are too large for AFL.
...
class AflRunnerCommon(object):
  ...
  def fuzzer_stderr(self):
    """Returns the stderr of the fuzzer.

    Reads it first if it wasn't already
  ...
  def afl_setup(self):
    """Make sure we can run afl.

    Delete any files that afl_driver needs to
  ...
  def set_arg(cls, afl_args, flag, value):
    """Sets the afl |flag| to |value| in |afl_args|.
  ...
  def run_afl_fuzz(self, fuzz_args):
    ...
    def check_error_and_log(error_regex, log_message_format):
      """See if error_regex can match in fuzz_result.output.
  ...
  def fuzz(self):
    ...
    Wrapper around run_afl_fuzz that performs one
    time setup.
    ...
    self.initial_max_total_time = 10  # TODO(mbarbella): Plumb timeout.
    #self.initial_max_total_time = (
      ...
      if not self.showmap_no_output_logged:
        ...
        logs.log_error(
            ('afl-showmap didn\'t output any coverage. Command: {0}\n'
             'Return code: {1}\n'
             'Time executed: {2}\n'
             'Output: {3}').format(showmap_result.command,
...
def run_single_testcase(fuzzer_runner, testcase_file_path):
