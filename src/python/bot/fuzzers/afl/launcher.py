    ...
    exists and sets configs based on it."""
        ...
        for libfuzzer_name, value in six.iteritems(libfuzzer_options.dict()):
            ...
            if afl_option.type == AflOptionType.ARG:
                self.additional_afl_arguments.append('%s%s' %
                                                     (afl_option.name, value))
            else:
...
class FuzzingStrategies(object):
  ...
  and to record the decision for StatsGetter to use later."""
    ...
    def __init__(self, target_path, strategy_dict=None):
        ...
        self.is_mutations_run = (self.candidate_generator !=
...
class AflRunnerCommon(object):
        ...
        try:
            with open(self.stderr_file_path, 'rb') as file_handle:
                stderr_data = utils.decode_to_unicode(
                    utils.read_from_handle_truncated(file_handle,
                                                     MAX_OUTPUT_LEN))
    ...
    """
        ...
        assert not testcase_path.isdigit(), (
            'We don\'t want to specify number of'
            ' executions by accident.')

        self.afl_setup()
        result = self.run_and_wait(additional_args=[testcase_path])
        print('Running command:',
              engine_common.get_command_quoted(result.command))
        if result.return_code not in [0, 1, -6]:
            logs.log_error('AFL target exited with abnormal exit code: %s.' %
                           result.return_code,
    ...
    """
        ...
        if num_retries > self.MAX_FUZZ_RETRIES:
            logs.log_error(
                'Tried to retry fuzzing {0} times. Fuzzer is likely broken'.
                format(num_retries))
      ...
      """
        ...
        while self.should_try_fuzzing(max_total_time, num_retries):
            ...
            if os.path.exists(cmplog_build_file):
                self.set_arg(fuzz_args, constants.CMPLOG_FLAG,
                             self.target_path)
            ...
            if engine_common.decide_with_probability(
                ...
                self.set_arg(fuzz_args, constants.QUEUE_OLD_STRATEGY_FLAG,
                             None)

            # Randomly select the MOpt mutator.
            if engine_common.decide_with_probability(
                    self.strategies.MOPT_PROB):
    ...
    """
        ...
        logs.log_error('CPU binding error encountered by afl-fuzz\n'
    ...
    """
        ...
        self.initial_max_total_time = (get_fuzz_timeout(
...
class AflRunner(AflRunnerCommon, new_process.UnicodeProcessRunner):
    ...
    def __init__(self,
        ...
        super().__init__(target_path, config, testcase_file_path,
                         input_directory, timeout, afl_tools_path,
                         strategy_dict)
...
class MinijailAflRunner(AflRunnerCommon, new_process.UnicodeProcessRunnerMixin,
    ...
    def __init__(self,
        ...
        minijail.MinijailProcessRunner.__init__(self, chroot,
                                                self.afl_fuzz_path)
    ...
    """
        chroot_rel_dir = os.path.relpath(corpus_directory,
    ...
    def generate_afl_args(self,
        ...
        return super().generate_afl_args(minijail_afl_input,
...
class Corpus(object):
    ...
    def element_paths(self):
        ...
        return set(element.path
                   for element in six.itervalues(self.features_and_elements))
...
def prepare_runner(fuzzer_path,
    ...
    if environment.get_value('USE_MINIJAIL'):
        ...
        runner = MinijailAflRunner(minijail_chroot,
                                   fuzzer_path,
                                   config,
                                   testcase_file_path,
                                   input_directory,
