...
class FuzzingStrategies(object):
  ...
  def __init__(self, target_path, strategy_dict=None):
    ...
    # Always do fast calibration. Makes the fastcal strategy functions obsolete.
    # This increases fuzzing randomness and reduces the start-up time.
    ...
    while self.should_try_fuzzing(max_total_time, num_retries):
      ...
      for schedule_opt, prob in self.strategies.SCHEDULER_PROBS:
        if rnd <= prob:
          ...
          rnd -= prob
      self.set_arg(fuzz_args, constants.SCHEDULER_FLAG, schedule)
      ...
      if self.strategies.EXPAND_HAVOC_PROB < engine_common.get_probability():
        environment.set_value(constants.EXPAND_HAVOC_ENV_VAR, 1)

      # Randomly only CMPLOG new finds.
      if self.strategies.CMPLOG_ONLY_NEW_PROB < engine_common.get_probability():
      ...
      if self.strategies.MOPT_PROB < engine_common.get_probability():
      ...
      for cmplog_level_opt, prob in self.strategies.CMPLOG_LEVEL_PROBS:
        if rnd <= prob:
          ...
          rnd -= prob
      self.set_arg(fuzz_args, constants.CMPLOG_LEVEL_FLAG, cmplog_level)
