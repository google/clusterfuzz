...
class BlackboxEngine(engine.Engine):
  ...
  Blackbox fuzzers are scripts which accept the following arguments:
    --app_path: The path to a test binary which will be invoked when testing.
  ...
  def fuzz(self, target_path, options, reproducers_dir, max_time):
   ...
   Returns:
    ...
    result = runner.run_and_wait(timeout=max_time, additional_args=args)
    ...
    for testcase_path in os.listdir(reproducers_dir):
      ...
      crash = engine.Crash(
          os.path.join(reproducers_dir, testcase_path), output, [],
