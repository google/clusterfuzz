...
class FuchsiaQemuLibFuzzerRunner(new_process.ProcessRunner, LibFuzzerCommon):
  ...
  def _pull_new_corpus_from_target_to_host(self, corpus_directories, artifact_prefix):
    ...
    self.device.fetch(
      self.fuzzer.data_path('crash*'), artifact_prefix)
    self.device.fetch(
      self.fuzzer.data_path('oom*'), artifact_prefix)
    self.device.fetch(
      self.fuzzer.data_path('timeout*'), artifact_prefix)
    self.device.fetch(
      self.fuzzer.data_path('leak*'), artifact_prefix)
