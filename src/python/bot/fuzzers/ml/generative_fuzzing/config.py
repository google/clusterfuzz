...
def yaml_train_parse(args):
    ...
    assert isinstance(hyperparameters, dict), "hyperparameters in conf.yaml is required"
    f.close()
    ...
    if args.batch_size is None:
        args.batch_size = DEFAULT_TRAIN_BATCH_SIZE if by_default else hyperparameters.get("batch_size")
    # Case for no --batch-size argument, by_default is False and argument is empty
    if args.batch_size is None:
        args.batch_size = DEFAULT_TRAIN_BATCH_SIZE

    if args.learning_rate is None:
        args.learning_rate = DEFAULT_LEARNING_RATE if by_default else hyperparameters.get("learning_rate")
    if args.learning_rate is None:
        args.learning_rate = DEFAULT_LEARNING_RATE

    if args.train_seqlen is None:
        args.train_seqlen = None if by_default else hyperparameters.get("train_seqlen")
    if args.train_seqlen is None:
        if utils.model_is(args.model_name, 'RNN'):
    ...
    args.validation_seqlen = DEFAULT_VALIDATION_SEQLEN if by_default else hyperparameters.get("validation_seqlen")
    if args.validation_seqlen is None:
        args.validation_seqlen = DEFAULT_VALIDATION_SEQLEN

    if args.n_epochs is None:
        args.n_epochs = DEFAULT_TRAIN_EPOCHS if by_default else hyperparameters.get("n_epochs")
    if args.n_epochs is None:
...
def train_parse_args():
    ...
    parser.add_argument('-c', '--config', help='Configuration file path', dest='config', default='conf.yml')
    ...
    parser.add_argument(
        '--batch-size', help='Batch size', type=int)
    parser.add_argument(
        '--debug', help='Print training progress', action='store_true', default=None)
    parser.add_argument(
        '--sliding-window', help='The window size to check if train loss is still decreasing', type=int)
    parser.add_argument(
        '--n-epochs', help='Number of train epochs', type=int)
    parser.add_argument(
        '--dropout-pkeep',
...
# The parameter to decide whether the prediction distribution close to uniform or argmax. 
DEFAULT_TEMPERATURE = 1.0

# If 0, we feed the whole sequence to the model to generate inference.
# If non 0, we feed the number of prefix bytes to the model to generate inference.
...
def generate_parse_args(batch_generation=False):
    ...
    parser.add_argument(
        '--temperature',
        help='Parameter to decide whether the model is close to uniform or argmax',
        type=float)
    parser.add_argument(
        '--insert-nums',
        help='Number of insertions in insert strategy',
        type=int)
