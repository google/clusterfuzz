...
class RNNGenerator:
  ...
  def generate(self):
    ...
    # We can check if the current position
    # needs to be replaced by predict value or not
    ...
    for i in range(self.batch_size):
      ...
      first_byte = random_file_info["first_byte"]
      ...
      if self.pos_change > 0:
        # Need to sample `pos_change` numbers
        # from predict_window_size to file_size - 1
        ...
        # Need to sample `file_size/(-pos_change)` numbers
        # from 1 to file_size - 1
        mod_positions = np.random.choice(
            file_size - 1, file_size // (-self.pos_change), replace=False) + 1
      ...
      for pos in range(max_length - 1):
        ...
        for i in range(self.batch_size):
          if self.pos_change == 0 or \
            (cur_modify_pos_index[i] < len_modify_positions[i] and \
              pos + 1 == modify_positions[i][cur_modify_pos_index[i]]) or \
            pos + 1 >= corpus_files_length[i]:
            if cur_modify_pos_index[i] < len_modify_positions[i] and \
              pos + 1 == modify_positions[i][cur_modify_pos_index[i]]:
    ...
    # So the length of new file is the minimal value of
    # max_length and the original file length.
    for i in range(self.batch_size):
