...
class RNNInsertGenerator:
    def __init__(self, hidden_layer_number, hidden_state_size, pkeep, model_weights_path,
        ...
        assert self.pos_change > 0, "pos_change must be > 0 in insert strategy"
        assert self.predict_window_size > 0, "predict_window_size must be > 0 in insert strategy"
    ...
    def generate(self):
        ...
        previous_bytes = np.zeros((self.batch_size, self.predict_window_size), dtype=np.int)
        ...
        for j in range(self.pos_change):
            ...
            for i in range(self.batch_size):
                for offset in range(self.predict_window_size):
                    previous_bytes[i][offset] = file_bytes_list[i][max(0, modify_positions[i][j] - self.predict_window_size + offset)]
                original_byte[i] = file_bytes_list[i][modify_positions[i][j]]

            prediction = self.model(previous_bytes, training=False)
            for i in range(self.batch_size):
                predict_byte = utils.sample_from_probabilities(prediction[i][-1].numpy())
                input_bytes[i][0] = predictions[i][j][0] = predict_byte
                if prediction[i][-1][original_byte[i]] > prediction[i][-1][predict_byte]:
                    truncate_len[i][j] = 0
                else:
            ...
            for insert_idx in range(1, self.insert_nums):
                ...
                for i in range(self.batch_size):
                    predict_byte = utils.sample_from_probabilities(prediction[i].numpy())
                    input_bytes[i][0] = predictions[i][j][insert_idx] = predict_byte
                    if truncate_len[i][j] == self.insert_nums and \
                        prediction[i][0][original_byte[i]] > prediction[i][0][predict_byte]:
        ...
        for i in range(self.batch_size):
            ...
            for j in range(len(file_bytes_list[i])):
                if cur_idx < len_modify_positions[i] and j == modify_positions[i][cur_idx]:
