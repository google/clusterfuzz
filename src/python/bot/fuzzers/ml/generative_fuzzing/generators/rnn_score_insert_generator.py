...
we will use [a1 a2 a3 0-255 a4 a5] as input and [a2 a3 0-255 a4 a5 a6] as output
...
class RNNScoreInsertGenerator:
    def __init__(self, hidden_layer_number, hidden_state_size, pkeep, model_weights_path,
                    input_dir, pos_change, predict_window_size, batch_size, temperature, insert_nums):
        self.model = RNNModel(
        ...
        self.ALPHA_SIZE = constants.ALPHA_SIZE
        self.insert_nums = insert_nums

        assert self.pos_change > 0, "pos_change must be greater than 0 in RNN score insert strategy."
        assert self.predict_window_size > 0, "predict_window_size must be greater than 0 in RNN score insert strategy."
        assert self.insert_nums > 0, "insert_nums must be greater than 0 in RNN score insert strategy."
    ...
    def generate(self):
        ...
        for i in range(self.batch_size):
            ...
            mod_positions = sorted(np.random.choice(file_size - 1, self.pos_change, replace=False))
            ...
            for pos_idx, pos in enumerate(mod_positions):
                ...
                input_bytes = np.zeros((self.ALPHA_SIZE, self.predict_window_size), dtype=np.int)
                target_bytes = np.zeros((self.ALPHA_SIZE, self.predict_window_size), dtype=np.int)
                # insert the byte right after pos.
                for c in range(self.ALPHA_SIZE):
                ...
                for insert_idx in range(self.insert_nums):
                    ...
                    insert_bytes[pos_idx][insert_idx] = predicted_byte = np.argmax(np.random.multinomial(1, prob))
                    ...
                    for c in range(self.ALPHA_SIZE):
                        input_bytes[c, : pos - st_pos + 1] = input_bytes[c, 1 : pos - st_pos + 2]
                        input_bytes[c, pos - st_pos + 1] = c
                        target_bytes[c, : pos - st_pos] = target_bytes[c, 1 : pos - st_pos + 1]
                        target_bytes[c, pos - st_pos] = c
            ...
            for j in range(file_size):
                ...
                if cur_insert_idx < self.pos_change and j == mod_positions[cur_insert_idx]:
