...
class VAEGenerator:
  ...
  we use m strategy and mask one byte among `seq_len` bytes
  to predict the corresponding byte.
  Use batch generation."""
  def __init__(self, batch_size, seq_len, model_weights_path,
               pos_change, input_dir, temperature):
    self.model = VAEModel(
    ...
    try:
      ...
      print('Incompatible model weights. '\
        'Please make sure the parameters are matched with training ones.')
      sys.exit(0)
  ...
  def generate(self):
    ...
    for i in range(self.batch_size):
      while True:
        random_file_info = utils.random_element_from_list(
            self.corpus_files_info)
      ...
      if file_size - self.seq_len >= self.pos_change:
        st_positions = np.random.choice(
            file_size - self.seq_len + 1, self.pos_change, replace=False)
      else:
        st_positions = np.minimum(np.maximum(0, file_size - self.seq_len),
                                  np.random.choice(
                                      file_size,
                                      self.pos_change,
                                      replace=False))
    ...
    for j in range(self.pos_change):
      ...
      for i in range(self.batch_size):
        input_bytes[i] = new_files_bytes[i][start_positions[i][j] : \
          start_positions[i][j] + self.seq_len]
