...
def main(args):
  ...
  diff_xy = not utils.model_is(model_name, "BERT") and not utils.model_is(
      model_name, "VAE")
  ...
  if utils.model_is(args.model_name, 'RNN'):
    ...
    model = VAEModel(batch_size=batch_size, seq_len=train_seqlen)
  else:
  ...
  for input_batch, expected_batch, epoch in utils.minibatch_sequencer(
    ...
    with tf.GradientTape() as tape:
      ...
      if utils.model_is(args.model_name, 'BERT_MASK'):
        ...
        loss_value = vae_loss(expected_batch, prediction[0], prediction[1],
                              prediction[2])
      else:
        loss_value = loss(expected_batch, prediction)
