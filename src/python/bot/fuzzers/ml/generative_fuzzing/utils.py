...
def minibatch_sequencer(raw_data, batch_size, sequence_size, nb_epochs, rtn_diff_xy = True):
...
def compute_batch_loss_and_acc_value(true_value, prediction, loss, all_stats=False):
    ...
    if all_stats:
        return pred_value, np.mean(loss_value, axis=1), np.mean(loss_value), np.mean(pred_value == true_value)
    else:
