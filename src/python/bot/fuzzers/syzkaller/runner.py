...
class AndroidSyzkallerRunner(new_process.ProcessRunner):
  ...
  def fuzz(self,
    ...
    logs.log('Current time: {}'.format(time.time()))
    pattern = 'report*'
    visited = set()
    for subdir, _, files in os.walk(constants.SYZKALLER_WORK_FOLDER):
      for file in files:
        ...
        # are grouped together in subfolders. subdir_file puts togetherthe
        # subfolder name (without './' (hence the [2:])) and reportN.
        subdir_file = subdir[len('./'):] + file
        if fnmatch.fnmatch(file, pattern) and subdir_file not in visited:
          visited.add(subdir_file)
          ...
          if crash_testcase_file_path:
            reproduce_arguments = {subdir_file}
