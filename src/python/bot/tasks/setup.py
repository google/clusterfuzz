...
def prepare_environment_for_testcase(testcase):
  ...
  # TODO(ochang): Remove once migration is completed. Testcases generated by the
  # new pipeline aren't compatible with the legacy launchers.
  if (environment.is_engine_fuzzer_job(testcase.job_type) and
      '%TESTCASE%' not in testcase.minimized_arguments):
    environment.set_value('USE_NEW_ENGINE_IMPL', True)
