...
class BuildSetupHostTest(unittest.TestCase):
  ...
  def test_remote_symbolized_build(self):
    """Test RemotesymbolizedBuild."""
    self.mock.stub().SetupSymbolizedBuild.return_value = (
        untrusted_runner_pb2.SetupBuildResponse(
            result=True,
            app_path='/release/bin/app',
            app_path_debug='/debug/bin/app',
            app_dir='/debug/bin',
            build_dir='/debug',
            build_url='https://build/url-release.zip'))

    build = build_setup_host.RemoteSymbolizedBuild(
        '/', 1337, 'https://build/url-release.zip',
        'https://build/url-debug.zip')

    self.assertTrue(build.setup())
    self.assertEqual(os.environ['APP_PATH'], '/release/bin/app')
    self.assertEqual(os.environ['APP_PATH_DEBUG'], '/debug/bin/app')
    self.assertEqual(os.environ['APP_DIR'], '/debug/bin')
    self.assertEqual(os.environ['BUILD_DIR'], '/debug')
    self.assertEqual(os.environ['BUILD_URL'], 'https://build/url-release.zip')
    self.assertEqual(os.environ['APP_REVISION'], '1337')
  ...
  def test_remote_production_build(self):
    ...
    self.assertEqual(os.environ['BUILD_URL'], 'https://build/url-stable.zip')
