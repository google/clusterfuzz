...
class StackAnalyzerTestcase(unittest.TestCase):
  ...
  def test_golang_panic_runtime_error_index_out_of_range_msan(self):
    """Test golang stacktrace with panic caused by index out of range with
    memory sanitizer."""
    data = self._read_test_data(
        'golang_panic_runtime_error_index_out_of_range_msan.txt')
    expected_type = 'Index out of range'
    expected_address = ''
    expected_state = ('http.(*conn).serve.func1\n'
                      'http.HandlerFunc.ServeHTTP\n'
